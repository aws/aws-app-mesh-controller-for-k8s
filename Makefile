...
helm-deploy: check-env manifests
	helm upgrade -i appmesh-controller config/helm/appmesh-controller --namespace appmesh-system --set image.repository=$(REPO) --set image.tag=$(VERSION) --set preview=$(PREVIEW) --set enableBackendGroups=$(ENABLE_BACKEND_GROUPS) --set sidecar.waitUntilProxyStarts=${WAIT_PROXY_READY}
